...
void main(){
  ...
  assert(enet_initialize == 0); // must be 0 if success
  ...
  assert(client); // must be non null
  ...
  if (enet_host_service (client, & event, 5000) > 0 && event.type == ENetEventType.ENET_EVENT_TYPE_CONNECT){
      ...
      while(i < 10) {
        i++;
        char[] s = "hello\0"; // use null terminated string (I guess)
      ...
      writefln ("Connection to server failed.");
  }
